...
var $ = jQuery.noConflict();
	...
	<div class="container row-offcanvas row-offcanvas-left">
		<div class="well column  col-lg-54  col-sm-6 span6" id="content">
		<?php
		if($_GET["team"]){
			...
			$tableEcho= $tableEcho.'<div style="border:1px solid black;overflow-y:hidden;overflow-x:scroll;"><table  class="sortable table table-hover" id="RawData" border="1"></div>';
...
$(document).ready(function() {
 $('#myCarousel').carousel({
	...
	<div class="container row-offcanvas row-offcanvas-left">
		<div class="well column col-sm-offset-6 span6" id="content">
		<?php
		if($_GET["team2"]){
			...
			mysql_select_db($database, $con);
			$query = 'select * from '.$matchScoutDatabase.' WHERE teamNum =' .$_GET["team2"];
			$result = mysql_query($query);
			$queryOne = 'select * from '.$pitDatabase.' WHERE TeamNumber =' .$_GET["team2"];
			$pitResult = mysql_query($queryOne);
			...
			if($color == "red"){
               ...
               $tableEcho= $tableEcho."<tr>";        
                    foreach ($row as $key => $value2){
                            if(!is_numeric($key)){
                                    if($key == "TeamNumber"){
                                            $value2= '<a href="teamData.php?team='.$value2.'">'.$value2.'</a>';
 }
                                    if($key == "MatchNumber"){
                                            $value2= '<a href="matchData.php?match='.$value2.'">'.$value2.'</a>';
                                    }
                               $tableEcho= $tableEcho."<td align='center'>".$value2."</td>";
                       }
               }        
               $tableEcho= $tableEcho."</tr>";                
            }
...
.rotate090 {
    ...
    transform: rotate(90deg);
			...
			<form action="" method="get">
			Enter Team Number: <input class="control-label"type="number" name="team2"  id="team2"  size="10" height="10" width="40"> 
			<button id="submit" class="btn btn-primary" onclick="">Display</button>
			<div class="row">
			<div class = "col-md-4">
			<h1> Team <?php echo($_GET["team2"]);?></h1>	
			<div class="box">			
			<div id="myCarousel" class="carousel slide" data-ride="carousel">
			  <ol class="carousel-indicators">
				<?php 
				$index = 0;
				while(file_exists("uploads/".$_GET["team2"]."-".$index.".jpg")==1){
						if($index == 0){	
							echo('<li data-target="#myCarousel" data-slide-to="'.$index.'" class="active"></li>');
						}
						else{
			  ...
			  <div class="carousel-inner" role="listbox">
				...
				while(file_exists("uploads/".$_GET["team2"]."-".$index.".jpg")==1){
						if($index == 0){	
							echo('<div class="item active">
									<img class="img-responsive" id="'.$_GET["team2"].'-'.$index.'" src="uploads/'.$_GET["team2"].'-'.$index.'.jpg">
								 </div>');
						}
						else{
							echo('<div class="item">
